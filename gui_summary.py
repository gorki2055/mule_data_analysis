...
def calculate_summary(file_path):
    try:
        ...
        # 3. Resampling for Time-Based Metrics
        ...
        if 'Current_Rotational_Speed' in df_res.columns:
            ...
            speed_kmh = np.where(speed_kmh > 100, 0, speed_kmh) # Filter spikes
        ...
        print(f"Max Current:       {df_res['Processed_Current'].max():.1f} A")
        print(f"Peak Power:        {(df_res['Battery_voltage'] * df_res['Processed_Current']).max()/1000:.2f} kW")
        print("-" * 30)
        ...
        available_temps = [c for c in temp_cols if c in df_res.columns]
        if available_temps:
            print("-" * 30)
