...
DATA_FILE = "00000001-695B41A6M.MF4.csv"
GEAR_RATIO = 6.09        #GEAR RATIO AFTER 4 JAN IS 6.09(QS) BEFOR 4  JNA 6.36
...
def load_and_process_data(file_path):
    ...
    if 'Battery_voltage' in df.columns and df['Battery_voltage'].mean() > 1000:
        ...
        # df['Battery_voltage_smooth'] = df['Battery_voltage'].rolling(window=40, center=True).median()
        df['Battery_voltage'] = medfilt(df['Battery_voltage'], kernel_size=15)
...
def resample_data(df):
    ...
    df_resampled = df_resampled.dropna(subset=['Battery_voltage', 'Battery_current1'])
    print(f"✓ Resampling complete: {len(df_resampled)} rows")
    
    # Add Ride_Mode back after resampling
    if 'Ride_Mode' in df.columns:
        ...
        df_resampled['Ride_Mode'] = mode_resampled
    
    # Add Processed_Current if available
    if 'Processed_Current' in df.columns:
        processed_series = df.set_index('timestamps')['Processed_Current']
        processed_resampled = processed_series.resample(RESAMPLE_RATE).mean()
        df_resampled['Processed_Current'] = processed_resampled
    
    # Add Charge_discharge_status if available
    if 'Charge_discharge_status' in df.columns:
        ...
        df_resampled['Charge_discharge_status'] = status_resampled
...
def calculate_metrics(df):
    ...
    df['Power_kW'] = df['Power_kW'].rolling(window=20, center=True).mean()
    # df['Power_kW_raw'] = (df['Battery_voltage'] * df['Battery_current1']) / 1000.0

    # Apply median filter with window size 5
    # df['Power_kW'] = medfilt(df['Power_kW_raw'], kernel_size=5)

    df['time_diff_hours'] = (df['timestamps'] - df['timestamps'].shift(1)).dt.total_seconds() / 3600.0
    ...
    # Distance calculation (multiple methods)
    metrics['distance_km'] = None
    
    # # Method 1: From DistanceTrip column
    # if 'DistanceTrip' in df.columns:
    #     df['Distance_km'] = df['DistanceTrip'].diff().clip(lower=0) / 1000
    #     metrics['distance_km'] = df['Distance_km'].sum()
    
    # Method 2: From rotational speed
    ...
    # # Method 3: From Speed column
    # elif 'Speed' in df.columns:
    #     df['distance_km'] = df['Speed'] * df['time_diff_hours']
    #     metrics['distance_km'] = df['distance_km'].sum()
...
#             # plt.plot(df['timestamps'], df['Ntc_Mos'], 
#             #          label="Permissible Discharge Current (A)", 
#             #          color=PLOT_STYLES['colors']['permissible'], 
#             #          linewidth=PLOT_STYLES['line_width_thin'], 
#             #          linestyle='-')
#             plt.title(f"Battery Current vs Permissible Current over Time\nStart: {df['timestamps'].iloc[0]}")
#             plt.xlabel("Time")
...
#             plt.gcf().autofmt_xdate()
#             plt.tight_layout()
#             plt.savefig(os.path.join(output_dir, "permissible_vs_battery_current.png"), dpi=PLOT_STYLES['dpi'])
#             plt.close()
